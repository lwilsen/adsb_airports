...
WORKSPACE = "airports"  # Workspace slug
...
def display_pdf(file_path):
    """
    Displays a PDF file in an iframe, allowing a user to ctrl-f to search the contents.
    """
...
def doc_to_pdf_and_display(doc_filepath):
    """
    Converts a Word document (docx) to a PDF and displays it inline.
    """
    # Ensure the input file is a .docx file (docx2pdf works best with .docx)
    # If your files are .doc, consider converting them to .docx first.
    if not doc_filepath.lower().endswith(".docx"):
        raise ValueError("Please provide a .docx file.")

    # Generate a PDF filename based on the docx filename
    pdf_filepath = os.path.splitext(doc_filepath)[0] + ".pdf"

    # Convert docx to pdf
    convert(doc_filepath, pdf_filepath)

    # Display the converted PDF
    display_pdf(pdf_filepath)


def parse_file(file):
    """
    Parses the uploaded file based on its type and returns its content as a string.
    ...
    if file_type in ["csv", "xls", "xlsx"]:
        df = pd.read_excel(file) if file_type in ["xls", "xlsx"] else pd.read_csv(file)
