...
try:
    cell_gdf = geo_dataframe[geo_dataframe[f"H3_{RESOLUTION}_cell"] == h3cell_id_list[0]]
    ...
    st.write(flights_in_hex_gdf.
            groupby(['flight','type']).
            size().
            reset_index(name = "count").
            sort_values(by = "count", ascending = False))

    selected_flight = st.selectbox("Select a flight to track",
                tuple(flights_in_hex))

    selected_type = st.selectbox("Select the type of aircraft",
                                tuple(types_in_hex))

    st.write(f'You selected flight: {selected_flight}')

    selected_f_gdf = geo_dataframe[(geo_dataframe[f"flight"] == selected_flight) & 
    ...
    flight_date_gdf['time_from_start'] = (flight_date_gdf['timestamp'] - 
                                        first_timestamp).dt.total_seconds() / 3600


    flight_date_geoms = flight_date_gdf[f'H3_{RESOLUTION}_cell'].apply(lambda x: cell_to_shapely(x))
    h3_gdf_plane = gpd.GeoDataFrame(data=flight_date_gdf, geometry=flight_date_geoms, crs=4326)

    geojson_obj_h3_gdf_plane = hexagons_dataframe_to_geojson(h3_gdf_plane,
